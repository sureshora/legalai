...
os.environ["OPENAI_API_KEY"] = os.getenv("OPENAI_API_KEY", config.get("OPENAI_API_KEY", ""))
os.environ["PINECONE_API_KEY"] = os.getenv("PINECONE_API_KEY", config.get("PINECONE_API_KEY", ""))
os.environ["PINECONE_ENV"] = os.getenv("PINECONE_ENV", config.get("PINECONE_ENV", "us-east-1"))
...
async def upload_file(file: UploadFile = File(...)):
    """Extract text from PDF and summarize legal documents."""
    try:
...
@app.post("/query/")
async def query_legal_ai(question: str):
...
async def contract_risk_analysis(file: UploadFile = File(...)):
    ...
    try:
        ...
        risk_analysis = openai_client.chat.completions.create(
            model="gpt-4",
            messages=[
                {"role": "system", "content": "Analyze legal contract risks."},
